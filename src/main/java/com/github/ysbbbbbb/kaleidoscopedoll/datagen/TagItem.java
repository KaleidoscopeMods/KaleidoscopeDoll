...
public class TagItem extends ItemTagsProvider {
    ...
    private static TagKey<Item> itemTagKey(String key) {
        return TagKey.create(Registries.ITEM, ResourceLocation.fromNamespaceAndPath(KaleidoscopeDoll.MOD_ID, key));
    }

    private static ResourceKey<Item> dollItem(String key) {
        return ResourceKey.create(Registries.ITEM, ResourceLocation.fromNamespaceAndPath(KaleidoscopeDoll.MOD_ID, key));
    }
    ...
    protected void addTags(HolderLookup.Provider provider) {
        ...
        this.tag(TIER_0_DOLLS).add(ModRegisterEvent.DOLL_ITEMS.stream()
                .filter(item -> !SPECIAL_TOOLTIPS.containsKey(BuiltInRegistries.ITEM.getKey(item)))
                .filter(item -> BuiltInRegistries.ITEM.getResourceKey(item)
